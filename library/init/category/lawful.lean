...
(bind_assoc : ∀ {α β γ : Type u} (x : m α) (f : α → m β) (g : β → m γ),
  x >>= f >>= g = x >>= λ x, f x >>= g)
-- helper lemma
(bind_bind_pure_comp_eq_seq : ∀ {α β : Type u} (f : m (α → β)) (x : m α), (f >>= λ f, x >>= pure ∘ f) = f <*> x :=
  by intros; rw ←bind_map_eq_seq; simp [(bind_pure_comp_eq_map _ _).symm])
...
(seq_assoc := by intros; simp [(bind_pure_comp_eq_map _ _).symm,
                                (bind_bind_pure_comp_eq_seq _ _).symm, bind_assoc, pure_bind])

export is_lawful_monad (bind_pure_comp_eq_map bind_bind_pure_comp_eq_seq bind_map_eq_seq pure_bind bind_assoc)
